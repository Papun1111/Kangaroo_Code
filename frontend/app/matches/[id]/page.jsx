...
export default function MatchDetailPage() {
    ...
    // Determine if the current user has permission to update the score
    const canUpdateScore = isAuthenticated && user && match && (
        ...
        user.role === 'UMPIRE' // Also allow users with a general UMPIRE role
    );
    ...
    return (
        <div>
            <div className="text-center mb-8">
                ...
                <p className="text-slate-600">{match.venue}</p>
                 <span className={`mt-2 inline-block px-3 py-1 text-sm font-semibold rounded-full ${
                    match.status === 'ONGOING' ? 'bg-red-200 text-red-800 animate-pulse' : 
