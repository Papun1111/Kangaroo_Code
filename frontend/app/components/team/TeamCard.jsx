...
const TeamCard = ({ team }) => {
    // IMPORTANT: Make sure your backend API response includes a property for the logo URL.
    // This code assumes that property is named 'logoUrl'.
    // Example team object structure: { id: 1, name: "Falcons", logoUrl: "https://...", ... }
            ...
            >
                ...
                <div className="p-8 flex flex-col justify-between h-full relative z-10">
                    ...
                    <div className="flex items-start justify-between mb-8">
                        ...
                        {/* --- UPDATED EMBLEM SECTION --- */}
                        {/* We keep the original container styling as the base/fallback */}
                        <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-800 flex items-center justify-center text-white font-black text-3xl shadow-lg group-hover:scale-110 group-hover:shadow-emerald-500/30 transition-all duration-300 overflow-hidden">
                            
                            {/* 1. The Base: Always render the Initial Letter */}
                            <span>
                                {team.name.charAt(0).toUpperCase()}
                            </span>

                            {/* 2. The Conditional Overlay: If logoUrl exists, render image on top */}
                            {logoUrl && (
                                <img
                                    src={logoUrl}
                                    alt={`${team.name} logo`}
                                    // 'absolute inset-0' makes it cover the gradient/letter completely.
                                    // 'bg-[#0a0a0a]' ensures transparent PNGs don't show the gradient underneath.
                                    className="absolute inset-0 w-full h-full object-cover bg-[#0a0a0a]"
                                    // Robustness: If image fails to load, hide this img tag, revealing the letter below.
                                    onError={(e) => {
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/5 group-hover:border-white/10 transition-colors">
                                ...
                                {/* Added optional chaining (?.) for safety if _count doesn't exist yet */}
                                <span className="text-gray-400 text-sm font-mono">
                                    {team._count?.members || 0} <span className="text-gray-600 uppercase text-[10px]">Agents</span>
                                </span>
                            </div>
                        </div>
                    </div>
